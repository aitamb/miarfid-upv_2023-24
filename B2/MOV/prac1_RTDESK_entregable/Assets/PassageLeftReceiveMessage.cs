...
enum PassageLeftActions { Consult }

[RequireComponent(typeof(RTDESKEntity))]

public class PassageLeftReceiveMessage : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        ConsultMsg.action = (int)PassageLeftActions.Consult;
        ...
        switch (Msg.Type)
        {
            case (int)UserMsgTypes.Action:
                ...
                if (gameObjectName == Msg.Sender.name)
                    ...
                    {
                        case (int)PassageLeftActions.Consult:
    ...
    {
        bool hit = Physics2D.BoxCast(transform.position, Vector2.one * 0.5f, 0f, Vector2.right, 1f, pacmanLayer);
        return hit;
    }
