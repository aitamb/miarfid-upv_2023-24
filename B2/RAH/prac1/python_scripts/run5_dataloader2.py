...
# Sample of trainset and testset using torch.utils.data.Dataset class and 
# augmentation with noise and RIR.
...
x, y = trainset[0]
torchaudio.save('out/run5_dataloader2/trainset_noise.wav', 
                torch.tensor(x)[None,...].float(), 16000)

trainset = TrainDataset(transform=[RIRAug(prob=1.0)])
x, y = trainset[0]
torchaudio.save('out/run5_dataloader2/trainset_rir.wav', 
                torch.tensor(x)[None,...].float(), 16000)