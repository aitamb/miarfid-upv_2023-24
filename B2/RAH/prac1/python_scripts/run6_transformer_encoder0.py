...
class FeedForward(torch.nn.Module):
    def __init__(self, d_model=512, d_ff=1024, dropout=0.1, **kwargs):
        ...
        self.ff = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            # Dropout: during training, randomly zeroes some of the elements of 
            # the input tensor with probability dropout using samples from a 
...
class SelfAttention(torch.nn.Module):
    def __init__(self, d_model, n_heads=8, d_head=64, dropout=0.1, **kwargs):
        ...
        # q for queries, k for keys, v for values
    ...
    def forward(self, x):
        ...
        # permute(desired order of dimensions)
        # (3, 2, 1, 0) means the 3rd will go on the 0th, 
        # the 2nd will go on the 1st, etc.
        q = q.permute(0, 2, 1, 3) # (bs, n_heads, seq_len, d_head)
        k = k.permute(0, 2, 3, 1) # (bs, n_heads, d_head, seq_len)
        v = v.permute(0, 2, 1, 3) # (bs, n_heads, seq_len, d_head)
...
# 2. Feature EXTRACTOR #########################################################
# The feature extractor is composed of a 
#    - log mel-spectrogram         and a
#    - linear layer
...
# The classification network is composed of an audio feature EXTRACTOR and a
# transformer encoder. 
...
class ClassificationNetwork(torch.nn.Module):
    ...
    def forward(self, x): 
        ...
        x = self.encoder(x)
        x = self.norm(x)
        # mean of the transformer encoder output (prediction)
        x = x.mean(1)
        x = self.out(x)
        return x
...
class TrainDataset(torch.utils.data.Dataset):
    def __init__(self, data_dir='data/data1/train', audio_len = 16000, 
                 transform=[identity]):        
        self.transform = transform
        self.audio_len = audio_len
        self.files = sorted( glob.glob(data_dir+'/*.wav') )  
        print("- TrainDataset lenght: ", len(self.files))
...
class TestDataset(torch.utils.data.Dataset):
    def __init__(self, data_dir='data/data1/test', audio_len = 16000):
        self.audio_len = audio_len       
        self.files = sorted(glob.glob(data_dir+'/*.wav'))
        print("- TestDataset lenght: ", len(self.files))
