...
class AudioTransformer(torch.nn.Module):
    ...
    def generate(self, x):
        ...
        self.eval()
        x = torch.nn.functional.pad(x.clone().detach(), (0, self.seq_len-len(x)), value=23)        
        if len(x.shape) == 1:
            x = x[None, :]
        y = [20,]
        with torch.no_grad():            
...
class RIRAug(object):
    def __init__(self, rir_dir='data/RIRS_NOISES_small/simulated_rirs_small/', prob=0.5):
        self.prob = prob
...
class TrainDataset(torch.utils.data.Dataset):
    def __init__(self, data_dir='data/data3/train', audio_len=4*16000, transform=[identity], seq_len=10):        
        self.transform = transform
        self.audio_len = audio_len
        self.seq_len = seq_len
        self.files = sorted( glob.glob(data_dir+'/*.wav') )
        print("Training data size:", len(self.files))        
...
class TestDataset(torch.utils.data.Dataset):
    def __init__(self, data_dir='data/data3/test', audio_len=4*16000, seq_len=10):
        self.audio_len = audio_len  
        self.seq_len = seq_len     
        self.files = sorted(glob.glob(data_dir+'/*.wav'))        
        print("Test data size:", len(self.files))        
...
print("- Model initial generation from random tensor (1, 64000):")
print(model.generate( torch.randn(1, 64000 ) ))

print("- Model initial generation of the training data example:")
print(model.generate(torch.from_numpy(xp)))
...
# some train samples
print("- Model final generation from random tensor (1, 64000):")
print(model.generate( torch.randn(1, 64000 ) ))

print("- Model final generation of the previous train example:")
print(model.generate(torch.from_numpy(xp)))
