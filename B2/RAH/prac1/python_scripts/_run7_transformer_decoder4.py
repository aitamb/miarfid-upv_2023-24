...
# The transformer decoder is a stack of self-attention, cross-attention and 
# feed-forward layers.
...
class Transformer(torch.nn.Module):
    def __init__(self, vocab_size=20, **kwargs):
        ...
        self.seq_len = kwargs['seq_len'] # 20 en este caso
...
# - The input is the 4 digits
# - The output is the list of digits and the sum.
...
class Digitsumset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Each number (not special tokens) is represented by a list
        # so number 12 would be [1, 2], that's why we need special tokens:
        # 20, 19, 21, 22 are special tokens
        # - 20 indicates the following is the number
        # - 19 indicates the following is the sum of the first two digits
        # - 18 indicates the following is the sum of the last two digits
        # - 21 indicates the following is the sum of all digits
        # - 22 indicates the end of the sequence
        # - 23 indicates empty symbol
        if len(x) == 1:
        ...
        x = torch.nn.functional.pad(torch.tensor(x), (0, self.seq_len-len(x)), 
                                    value=23)
...
model = Transformer(vocab_size=24, d_model=128, nb_layers=8, d_ff=256, 
                    n_heads=8, d_head=64, dropout=0.1, seq_len=20)
print("- Training data example:")
xp, yp = trainset.__getitem__(0)
print('', xp, '\n -> \n', yp)

print("- Model initial generation (from number 1234):")
...
# some train samples
print("- Some train samples predictions:")
print("from number 14896")
print(model.generate([4, 8, 9, 6]))
print("from number 921")
print(model.generate([9, 2, 1]))

# some test samples
print("- Some test samples predictions:")
print("from number 6252")
print( model.generate([6, 2, 5, 2]) )
print("from number 4684")
