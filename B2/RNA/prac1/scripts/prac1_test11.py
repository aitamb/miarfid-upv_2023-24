# PRACTICA 1
# Probar DA, LR Scheduling y BN para mejorar la accuracy en MNIST
...
test_transform = transforms.Compose(
                    [
                    transforms.ToTensor(), # de pil a tensor, esta funcion ya lo normaliza
                    ])
# no se testea con DA porque quieres ver el resultado de la red con datos reales
# en algunos problemas si se podria hacer para hacer la estimacion de la accuracy mas robusta
# estimador bayesiano 

class MNIST_dataset(Dataset):

    def __init__(self, partition = "train", transform=None):
        # nuevo parámetro, transform

        print("\nLoading MNIST ", partition, " Dataset...")
        self.partition = partition
        self.transform = transform
        if self.partition == "train": # solo se transforma el train
    ...
    def __getitem__(self, idx):
        ...
        # image = image para usarlo sin DA (?)
...
# planificador on plateau
scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=0, threshold=0.001, cooldown=1, verbose=True)
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
      with tqdm(iter(test_dataloader), desc="Test " + str(epoch), unit="batch") as tepoch: 
            ...
            scheduler.step(test_loss) # STEP DEL SCHEDULER
